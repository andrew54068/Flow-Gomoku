...
describe("Matcher", () => {
  ...
  test("get index by address", async () => {
    ...
    const [aliceWaitingIndex, scriptError3] = await executeScript('Matcher-get-waiting-index-by-address', [alice])
    expect(scriptError3).toBeNull()
    expect(aliceWaitingIndex).toBe(0)
    
    const [bobWaitingIndex, scriptError4] = await executeScript('Matcher-get-waiting-index-by-address', [bob])
